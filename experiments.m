...
is_time = zeros(length(target_error), rounds);
...
bp_time = zeros(length(target_error), rounds);
...
for r = 1 : rounds
    ...
    for k = 1 : length(target_error)
        ...
        disp("- Time = " + time);
        disp("- Normalized iterations = " + iterations / edges);
...
save("data", "a", "b", "v1", "v2", "n", "lambda", "alpha", "bp_max_iter", "is_max_iter", "rounds", "eta", "target_error", "is_iter", "is_erro", "is_flip", "is_time", "is_fail", "bp_iter", "bp_erro", "bp_time", "bp_fail");